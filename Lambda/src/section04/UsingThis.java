/*
 * [클래스의 멤버 사용]
 * - 람다식에서 this는 내부적으로 생성도는 익명 개게의 참조가 아니라
 *   람다식을 실행한 객체의 참조이다.
	...
	class Inner {
		...
		void method() {
			MyFunctionalInterface func = () -> {
				...
				// 람다식에 선언된 필드를 참조하지 못한다.
				// System.out.println("outterField: this.outterField=" + this.outterField);
				
				System.out.println("outterField:" + innerField); // 람다식에 선언된 필드를 참조
				System.out.println("innerField: this.innerField=" + this.innerField); // Inner.innerField 참조
			};
